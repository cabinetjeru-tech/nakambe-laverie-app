 ...
 *   npm run db:seed
 *
 * - Crée le compte super-administrateur (SEED_ADMIN_EMAIL / SEED_ADMIN_PASSWORD).
 * - Si SEED_DEMO !== "false" : crée des formations, formateurs et un apprenant de démonstration,
 *   tous marqués `isDemo = true` et avec des emails en @demo.nourou-academy.local.
 *   Aucun avis ni témoignage n'est inventé.
 * Idempotent : peut être relancé sans créer de doublons.
 */
import { PrismaClient, type Prisma } from "@prisma/client";
import bcrypt from "bcryptjs";
import { randomBytes } from "node:crypto";
import { categories, courses, faqs, trainers } from "./seed-content";
import { indexLesson } from "../src/lib/rag/ingest";

const prisma = new PrismaClient();

async function main() {
  const adminEmail = (process.env.SEED_ADMIN_EMAIL || "admin@nourou-academy.local").toLowerCase();
  let adminPassword = process.env.SEED_ADMIN_PASSWORD;
  const existingAdmin = await prisma.user.findUnique({ where: { email: adminEmail } });
  if (!existingAdmin) {
    if (!adminPassword) adminPassword = randomBytes(9).toString("base64url") + "7a";
    ...
    console.log(`✔ Super-administrateur créé : ${adminEmail} / ${adminPassword}  (changez ce mot de passe après connexion)`);
  } else console.log(`• Super-administrateur existant : ${adminEmail}`);
